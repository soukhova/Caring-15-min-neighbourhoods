...
Working with the parcel to dest TTM - walk mode.
```{r library-setup}
library(biscale)
library(cowplot)
library(sf)
...
```{r}
load("data-raw/parcels/R_PARCELS_2020.rda") #add the census id to the parcels, then the ttm
...
care_dest <- care_dest |> 
  mutate(Care_Category_Specific = case_when(
    Care_Category_Specific  == "Daycare_EarlyON" ~ "Daycare | EarlyON",
    Care_Category_Specific == "Community_Centre" ~ "Community | Rec. Centre",
    Care_Category_Specific == "Daycare_Centre" ~ "Daycare | EarlyON",
    Care_Category_Specific == "Recreation_Centre" ~ "Community | Rec. Centre",
    Care_Category_Specific == "School" ~ "School",
    Care_Category_Specific == "Long-Term_Care_Home" ~ "LTC | Ret. Home",
    Care_Category_Specific == "Retirement_Home" ~ "LTC | Ret. Home",
...
test_all <- ttm_care_walk_15 |>
  sp_avail_detailed(o_id = from_id,
                    ...
                    opp = f, 
                    r = f, f = f, alpha = 1) |> 
  group_by(from_id, Care_Category, Care_Category_Specific) |> summarize(V_isc = sum(V_ij, na.rm = TRUE)) 

access_15 <- access_15 |> left_join(test_all, by=c("from_id", "Care_Category", "Care_Category_Specific")) |> ungroup()
rm(test_all)
access_15$V_isc |> sum(na.rm = TRUE) #sums to 2206 opportunities -- out of 2225 total opportunities. some opportunities cannot be reached within 15 min walk! 
```
...
access_15_zeros <- 
  data.frame(from_id = rep(R_PARCELS_CENTS_2020$ID, length(access_15$Care_Category_Specific |> unique())),
           ...
           Care_Category_Specific = rep(c("Park", "Daycare | EarlyON", "Convenience Store", "School",
                                          "LTC | Ret. Home", "Senior Centre",
                                          "Bank or ATM", "Library", 
                                          "Grocer", "Dentist", 
                                          "Pharmacy", "Post Office",
                                          "Community | Rec. Centre", "Hospital or Clinic"), length(R_PARCELS_CENTS_2020$ID)),
           copp_isc = rep(0, length(R_PARCELS_CENTS_2020$ID)*length(c("Park", "Daycare | EarlyON", "Convenience Store", "School",
                                          "LTC | Ret. Home", "Senior Centre",
                                          "Bank or ATM", "Library", 
                                          "Grocer", "Dentist", 
                                          "Pharmacy", "Post Office",
                                          "Community | Rec. Centre", "Hospital or Clinic"))),
           copp_isc_w = rep(0, length(R_PARCELS_CENTS_2020$ID)*length(c("Park", "Daycare | EarlyON", "Convenience Store", "School",
                                          "LTC | Ret. Home", "Senior Centre",
                                          "Bank or ATM", "Library", 
                                          "Grocer", "Dentist", 
                                          "Pharmacy", "Post Office",
           ...
           V_isc = rep(0, length(R_PARCELS_CENTS_2020$ID)*length(c("Park", "Daycare | EarlyON", "Convenience Store", "School",
                                          "LTC | Ret. Home", "Senior Centre",
                                          "Bank or ATM", "Library", 
                                          "Grocer", "Dentist", 
                                          "Pharmacy", "Post Office",
...
rm(access_15_zeros)
#check
access_15$V_isc |> sum(na.rm=T)
```

Aggregating time!
```{r, eval=FALSE}
#ID = care destination ID (in care_dest), GeoUID is id of the DA, from_id is parcel ID
save(access_15, file= file.path(here("data/access_15.rda")), compress = "xz")
```
```{r}
load(file= file.path(here("data/access_15.rda")))
```
...
#check
access_15_bycategory$V_ic |> sum(na.rm=T)
```

```{r}
#by DA and mean:
access_15_DA_2 <- access_15 |>
  ...
  summarise(copp_iscDA_med = median(copp_isc), #the median number of opportunities (all) that can be reached from each parcel in a DA
            ...
            V_iscDA = sum(V_isc), #the number of total spatially available opportunities (all) that can be reached from each parcel in a DA
            vv_iscDA = sum(V_isc)/first(Population), #the per-capita number of total spatially available opportunities (all) that can be reached from each parcel in a DA
...
access_15_DA_2 <- access_15_DA_2 |> left_join(R_PARCELS_CENTS_count_GeoUID, by="GeoUID")
access_15_DA_2 <- left_join(HAM_census_21 |> select(c(GeoUID)), access_15_DA_2, by=c("GeoUID"))
...
access_15_bycategory_DA <- access_15 |>
  ...
  summarise(copp_ic_med = median(copp_isc), #the median number of opportunities that can be reached by category from each parcel in a DA
            ...
            V_ic = sum(V_isc), #the number of total spatially available opportunities that can be reached by category from each parcel in a DA
            vV_ic = sum(V_isc)/first(Population), #the per-capita number of total spatially available opportunities that can be reached by category from each parcel in a DA
            Population = first(Population),
            Dwellings = first(Dwellings),
            LICO_AT_TOTAL = first(LICO_AT_TOTAL),
            COMMUNITY_ = first(COMMUNITY_) ) |>
...
access_15_byspecificcategory_DA <- access_15 |>
  ...
  summarise(copp_isc_med = median(copp_isc), #the median number of opportunities that can be reached by specific category from each parcel in a DA
            copp_isc_sd = sd(copp_isc), 
            V_isc = sum(V_isc), #the number of total spatially available opportunities that can be reached by specific category from each parcel in a DA
            vv_isc = sum(V_isc)/first(Population), #the per-capita number of total spatially available opportunities that can be reached by specific category from each parcel in a DA
            Population = first(Population),
            Dwellings = first(Dwellings),
            LICO_AT_TOTAL = first(LICO_AT_TOTAL),
            COMMUNITY_ = first(COMMUNITY_) ) |>
...
#checks
# access_15_DA_2 |> summary()
access_15_bycategory_DA |> summary()
access_15_DA_2$V_iscDA |> sum(na.rm=T)
access_15_bycategory_DA$V_ic |> sum(na.rm=T)
access_15_byspecificcategory_DA$V_isc |> sum(na.rm=T)

#RECALL, categories are either 
# - "Dependent-centric" and include "Community | Rec. Centre", "Daycare | EarlyON", "LTC | Ret. Home", "Park", "School", "Senior Centre"
...
```
```{r}
access_15_byspecificcategory_DA |> group_by(Care_Category_Specific) |> summarise(V_isc = sum(V_isc))
...
```{r}
#calculating entropy, here I do it by the 4 categories for each parcel and then aggregate to DA. Another way is by the 14 specific categories. For now - keep it simple and just focus on the 4 categories. This assumes that all specific types are the SAME within categories.
step1 <- access_15  |> group_by(from_id) |> 
  summarise(OPPi = sum(copp_isc, na.rm=T),
            Vi = sum(V_isc))

step2 <- access_15 |> left_join(step1, by="from_id")

step2 <- step2 |> mutate(
  top1_copp = (copp_isc) / OPPi * log((copp_isc) / OPPi),
  top1_copp = ifelse(is.nan(top1_copp), 0, top1_copp),
  
  top1_V= (V_isc) / Vi * log((V_isc) / Vi),
  top1_V = ifelse(is.nan(top1_V), 0, top1_V),
  
  entropy_copp_isc = sum(top1_copp) / -log(4),
  entropy_V_isc = sum(top1_V) / -log(4))  |> 
  group_by(GeoUID) |>
  summarise(entropy_copp_isc = mean(entropy_copp_isc, na.rm=T) + sd(entropy_copp_isc, na.rm=T),
            entropy_V_isc = mean(entropy_V_isc, na.rm=T) + sd(entropy_V_isc, na.rm=T))
...
all_cats <- access_15_bycategory_DA |> 
  ...
  group_by(GeoUID) |>
         summarise(summed_copp_all = sum(copp_all),
                   ...
                   entropy_copp_isc = first(entropy_copp_isc),
                   entropy_V_isc = first(entropy_V_isc)) |>
...
all_cats$summed_Vic |> sum() #only 2206  opps can be reached out of the total 2225, so that's what the numbers are based on.
care_dest|> nrow()

all_cats <- left_join(HAM_census_21 |> select(c(GeoUID)), all_cats, by=c("GeoUID"))
```
<!--
SUPERCEEDED BY THE PLOTS IN 03-CLUSTERING..qmd
...
all_cats_copp_plot <- tm_shape(ham_bay, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0) +
  ...
  tm_polygons("Care_Access_Status",
              ...
              title = "Number of care destinations spatially accessible within 15 min. walk",
...
tmap_save(all_cats_copp_plot, file= file.path(here("figures/all_cats_copp_plot.tiff")), dpi=600)
```
...
all_cats_entropy_copp_plot <- tm_shape(ham_bay, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0) +
  tm_shape(hydro_p_LakeOntario, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0)+
  tm_shape(all_cats) +
  tm_polygons("entropy_copp_isc",
              border.alpha = 0.2,
              palette = "Reds",
              title = "Diversity of care destinations spatially accessible within 15 min. walk",
              legend.is.portrait = TRUE) +
  tm_shape(Community_Boundaries) + tm_polygons(alpha=0)+
  tm_scale_bar(position = c("left", "bottom"), breaks = c(0, 1, 5, 10, 15)) +
  tm_compass(position = c("right", "top"), size=1.0)+
...
tmap_save(all_cats_entropy_copp_plot, file= file.path(here("figures/all_cats_entropy_copp_plot.tiff")), dpi=600)
```
--->
...
all_cats_vvi_plot <- tm_shape(ham_bay, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0) +
  ...
  tm_polygons("vvi",
              ...
              title = "Per capita spatially available care destinations within 15 min. walk",
              legend.is.portrait = TRUE) +
  tm_shape(Community_Boundaries) + tm_polygons(alpha=0)+
  tm_scale_bar(position = c("left", "bottom"), breaks = c(0, 1, 5, 10, 15)) +
  tm_compass(position = c("right", "top"), size=1.0)+
...
tmap_save(all_cats_vvi_plot, file= file.path(here("figures/all_cats_vvi_plot.tiff")), dpi=600)
```
...
all_cats$Care_Access_Status |> unique()
```
  ...
  mutate(bivariate_entropy = 
           case_when( ((Care_Access_Status == "[04-25 des.) Caring - complete (2 to 4 options per cat.)"  |
                       Care_Access_Status =="[04-25 des.) Caring - very complete (4 or more options per cat.)" |
                         ...
                         entropy_copp_isc >= 0.7418) ~ "Caring, complete and amenity diverse",  #the median diversity,
                      
                     ((Care_Access_Status == "[04-25 des.) Caring - complete (2 to 4 options per cat.)"  |
                       Care_Access_Status =="[04-25 des.) Caring - very complete (4 or more options per cat.)" |
                         Care_Access_Status =="[25+ des.) Very caring - very complete (4 or more options per cat.)") &
                        entropy_copp_isc < 0.7418) ~ "Caring, complete but not amenity diverse", 
                     
                     ((Care_Access_Status == "[0-04 des.) Not caring " |
                       Care_Access_Status == "[04-25 des.) Caring ")  &
                        entropy_copp_isc >= 0.7418) ~ "Not caring or not complete, but amenity diverse", 
                     
                     ((Care_Access_Status == "[0-04 des.) Not caring "  |
                       Care_Access_Status == "[04-25 des.) Caring ")  &
                        entropy_copp_isc < 0.7418) ~ "Not caring, complete, or amenity diverse"))
...
tmap_save(all_cats_bivariate_entropy_copp_plot, 
          file= file.path(here("figures/all_cats_bivariate_entropy_copp_plot.png")),
          dpi=300)
```

```{r, eval=FALSE}
care_categories_plot <- 
  tm_shape(Community_Boundaries, bbox=Community_Boundaries) + tm_polygons(col = "white", border.alpha=0)+
  tm_shape(ham_bay, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0) +
  tm_shape(hydro_p_LakeOntario, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0)+ 
    tm_shape(Community_Boundaries, bbox=Community_Boundaries) + tm_polygons(col = NA, alpha=0, border.col = "black", lwd=1)+
  tm_shape(care_dest) + tm_symbols(shape="Care_Category_Specific", size=0.03, border.lwd = 0, shapes = c(21,22,23,24,25,15), col = "#E6AB02", title.shape = "")+
  # tm_shape(care_dest) + tm_dots("Care_Category_Specific", palette = RColorBrewer::brewer.pal(n = 4, name = 'Dark2'),
  #                              size=0.025, title = "")+
  ...
  tm_layout(bg.color = "grey", panel.label.bg.color = "white", legend.position = c("right", "top"), scale = 1.1, legend.bg.color = "white", legend.bg.alpha = 0.65,
...
#care_categories_plot
tmap_save(care_categories_plot,
          file= file.path(here("figures/care_categories_plot.png")),
          dpi=600)
```
...
#  tm_shape(ham_bay, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0) +
#   tm_shape(hydro_p_LakeOntario, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0)+
#   tm_shape(access_15_DA_2 |> mutate(popden = Population/Dwellings)) +
#   tm_polygons("popden",
#               border.alpha = 0.2,
...
#    tm_layout(bg.color = "grey", panel.label.bg.color = "white", legend.position = c("right", "top"), legend.text.size = 0.5, legend.title.size = 0.7, legend.bg.color = "white", legend.bg.alpha = 0.4)
# 
# pop_descriptive_plots <- tmap_arrange(parcels_DA_plot, poprate_DA_plot, nrow=2)
# 
# tmap_save(pop_descriptive_plots, 
#           file= file.path(here("figures/pop_descriptive_plots.tiff")) , asp = 1/0.6,
#           dpi=600)
```
