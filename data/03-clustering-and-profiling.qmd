...
```{r}
library(cowplot) # Streamlined Plot Theme and Plot Annotations for 'ggplot2' # Streamlined Plot Theme and Plot Annotations for 'ggplot2'
...
df <- access_15_wide |>
  transmute(scluster = factor(scluster),
            ...
            Age_0to14_and_Age_65plus =( `v_CA21_11: 0 to 14 years` + `v_CA21_251: 65 years and over`) / (`v_CA21_11: 0 to 14 years` + `v_CA21_68: 15 to 64 years` + `v_CA21_251: 65 years and over`),
...
Notice that super cluster 3 () makes most of the node with the highest prevalence of poverty (LICO > 10.9%). What is the accessibility profile of this cluster?

It is high diversity, with multiple Grocery with multiple Grocery- and Dependant-centric opportunities, and at least some Errand-centric opportunities:
```{r}
access_15_wide |>
  select(scluster, starts_with("total_"), div) |>
  rename_with(.fn = ~str_replace(.x, "total_", "")) |>
...
It is clear that LICO alone is not sufficient to sharply profile the clusters. Ask Anastasia to choose other variables. 