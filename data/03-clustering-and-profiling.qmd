...
pop_DA_plot_2 <-
 tm_shape(ham_bay, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0) +
  ...
  tm_compass(position = c("left", "top"))+
   tm_layout(bg.color = "grey", panel.label.bg.color = "white", legend.position = c("right", "top"), legend.bg.color = "white", legend.bg.alpha = 0.4)
...
tmap_save(pop_DA_plot_2,
          file= file.path(here("figures/pop_descriptive_plot_onlypop.png")),
          dpi=300)
```
...
dep_copp_plot <- 
  tm_shape(hydro_p_LakeOntario, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0)+
  tm_shape(access_15_DA |> mutate(med_dep_copp = ifelse(med_dep_copp == 0, NA, med_dep_copp))) +
  ...
  tm_scale_bar(position = c("left", "bottom"), breaks = c(0, 1, 5, 10, 15)) +
  tm_compass(position = c("left", "top"), size=1.0)+
  tm_layout(legend.bg.color = "white", legend.bg.alpha = 0.4, scale=0.8,
            bg.color = "grey",
            legend.position = c("right","top"))

health_copp_plot <- 
  tm_shape(hydro_p_LakeOntario, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0)+
  tm_shape(access_15_DA |> mutate(med_health_copp = ifelse(med_health_copp == 0, NA, med_health_copp))) +
...
groc_copp_plot <-
  tm_shape(hydro_p_LakeOntario, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0)+
  tm_shape(access_15_DA |> mutate(med_groc_copp = ifelse(med_groc_copp == 0, NA, med_groc_copp))) +
...
err_copp_plot <- 
  tm_shape(hydro_p_LakeOntario, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0)+
  tm_shape(access_15_DA |> mutate(med_err_copp = ifelse(med_err_copp == 0, NA, med_err_copp))) +
...
```{r, eval=FALSE}
all_cats_entropy_copp_plot <- 
  tm_shape(hydro_p_LakeOntario, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0)+
  tm_shape(access_15_DA |> mutate(med_div = ifelse(med_div == 0, NA, med_div))) +
  tm_polygons("med_div",
              border.alpha = 0.2,
              style = "quantile", n=4, #are the quantiles, but are also similar to the clustered profiles
              #label = c("[0 to 0.49) Not complete", "[0.49 to 0.85) Complete", "[0.85 to 1] Very complete"),
              palette = "Greys",
              title = "Diversity of care destinations within \na 15-minute walk for a median parcel",
              legend.is.portrait = TRUE,
              colorNA = "white",
              textNA = 0,
              border.col = NULL) +
...
freq_sc_renamed <- ggplot(access_15_wide |> 
  ...
  scale_fill_manual(name = "Neighbourhood typologies",
                    values = c("#44ce1b","#bbdb44", "#f7e379", "#f2a134","#e51f1f"),
                    labels = c("Very caring - very complete","Caring - complete","Somewhat caring - not complete","Somewhat caring - complete","Not caring - not complete"))
```
