...
Let's work with the TTM that we already have ! Only destinations within 15 minutes or less, for walking mode.
```{r library-setup}
library(biscale)
library(cowplot)
library(sf)
...
all_cats |> group_by(COMMUNITY_) |> 
  summarize(DApop_served_all_categories = sum(Population*Complete, na.rm=T),
            DApopperc_served_all_categories = sum(Population*Complete, na.rm=T)/sum(Population))


one_cat <- access_15 |> group_by(from_id, Care_Category)|>
  summarise(Complete = ifelse(all(c_opp > 0 ), 1, 0),
            c_opp = sum(c_opp), 
            V_ic = sum(V_ic),
            Population = first(Population),
...
one_cat_communities <- one_cat |> group_by(COMMUNITY_, Care_Category) |> 
  summarize(DApop_served_1_category = sum(Population*Complete, na.rm=T),
            DApopperc_served_1_category = sum(Population*Complete, na.rm=T)/sum(Population))

one_cat_communities |> filter(COMMUNITY_ == "Hamilton-Central")
one_cat_communities |> filter(Care_Category == "Grocery_Centric") 
```
```{r}
all_cats <- left_join(HAM_census_21 |> select(c(GeoUID)), all_cats, by=c( "GeoUID" = "from_id"))
```
```{r message=FALSE}
tmap_mode("view")
...
```
```{r}
one_cat <- left_join(HAM_census_21 |> select(c(GeoUID)), one_cat, by=c( "GeoUID" = "from_id"))
```
