...
ttm_care_walk_15 <- ttm_care_walk_parcel |> 
  ...
  left_join(
    care_dest |> st_drop_geometry() |> select(c("care_ID", "Care_Category", "Care_Category_Specific", "Care_Category_Speciality", "Dest_count_weighted")),  by = c("to_id" = "care_ID")) |>
  mutate(f = 1)
...
access_15 <- ttm_care_walk_15 |> 
  ...
  summarize(GeoUID = first(GeoUID),
            copp_isc = n(),
            copp_isc_w = sum(Dest_count_weighted)) 
...
test_all <- ttm_care_walk_15 |>
  sp_avail_detailed(o_id = from_id,
                    ...
                    opp = Dest_count_weighted, #instead of just being vector of 1
                    r = f, f = f, alpha = 1) |> 
