...
`r word_comment("We should focus this discussion on: defining mobility of care and connecting it to the 15-minute neighbourhoods, the importance of walk-accessibility, and socio-demographic/economic characteristics associated with this overlap (esp. single parent households, areas with a high proportion of children, low-income prevalence).", " ")`
...
3)	Mid-size typical Canadian city case study. How frequently do we find these caring neighbourhoods (potential - accessibility)? Clustering methodology to determine this.
4)	Who enjoys them and who does not? Clustering methodology to determine these profiles.
  - i.e. with LICO > X + Prevalence of children >Y + walking accessibility score > U + diversity score > W = “caring neighbourhood”
...
- Next: we describe Hamilton's land-use context; communities which are urban, rural, suburban. Also describe the residential parcels summary statistics, ~140,000 points, aggregated at the level of Canadian Census DA. See parcel, population and population per parcel plots in @fig-Fig1:
...
```
Some of the socio-economic and demographic variables that characterise accessibility and the diversity of accessibility to care destinations are pictured in @fig-Fig2:
...
Here we will describe the estimated 15-minute walk travel times using {r5r}, from all residential parcel points to all care destinations assuming default walking speed (3.6km/hr) and OSM road network for Hamilton. Provide summary statistics.
...
## Methods
Cumulative opportunity method; 15-minute normative catchment for walk mode is used to represent the 15-minute neighbourhood concept. This has been done in previous literature such as @vale2023accessibility, @graellsCityCitiesMeasuring2021 and @gaglioneUrbanAccessibility15minute2022.

In this work, the cumulative opportunity score is calculated for each parcel $i$ and aggregated by care category $c$ and takes following mathetical form:
$$
...
Accessibility per category @fig-Fig4:
...
#| label: fig-Fig5
#| fig-cap: "The diversity measure based on the proportion of care category spatial accessibility (figure 4). Scale is in tertiles. The fifth input into the superclusters. Basemap shapefiles are retrieved from the 2021 Canadian census [@governmentofcanadaCensusPopulation2023], the Open Data Hamilton Portal [@opendatahamiltonCityBoundary2023] and the USGS [@greatlakesUSGS2010]."
knitr::include_graphics(glue::glue(here::here(), "/figures/all_cats_entropy_copp_plot.png")) 
```

Superclusters based on accessibility per category and diversity (the two above figures). 5 clusters are identified, see their dendrogram (right) and grid map (left) in @fig-Fig6 :
